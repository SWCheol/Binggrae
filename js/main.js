...
let itemSlide = function(slide) {
    ...
    btnPrev.addEventListener('click', function() {
        ...
        if(slide.style.right != '0px')
            slide.style.right = parseFloat(slide.style.right) - parseFloat(liWidth) + 'px';
    });

    btnNext.addEventListener('click', function() {
        let liWidth = slide.children[0].clientWidth;
        if(slide.style.right != (slide.children.length - Math.round(slide.clientWidth / liWidth)) * liWidth + 'px')
            slide.style.right = parseFloat(slide.style.right) + parseFloat(liWidth) + 'px';
    });
}
